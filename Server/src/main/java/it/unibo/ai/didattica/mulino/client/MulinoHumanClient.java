...
public class MulinoHumanClient extends MulinoClient {
    ...
    public MulinoHumanClient(State.Checker player) throws IOException {
		super(player);
	 ...
	 */
    private static Action stringToAction(String actionString, State.Phase fase) {
        if (fase == State.Phase.FIRST) { // prima fase
			Phase1Action action;
			...
			return action;
        } else if (fase == State.Phase.SECOND) { // seconda fase
			Phase2Action action;
