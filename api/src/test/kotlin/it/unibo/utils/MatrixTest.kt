...
class MatrixTest {
    ...
    @Test
    fun getTest() {
        val matrix = Matrix(3, 4, { xIndex, yIndex -> xIndex + yIndex })
        val expectedRow1 = listOf(0, 1, 2, 3)
        val expectedRow2 = listOf(1, 2, 3, 4)
        val expectedRow3 = listOf(2, 3, 4, 5)

        Assert.assertArrayEquals(expectedRow1.toTypedArray(), matrix.get(0).toTypedArray())
        Assert.assertArrayEquals(expectedRow2.toTypedArray(), matrix.get(1).toTypedArray())
        Assert.assertArrayEquals(expectedRow3.toTypedArray(), matrix.get(2).toTypedArray())
